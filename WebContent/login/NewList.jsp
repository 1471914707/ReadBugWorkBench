...
<div class="Layer1" align="center">
	<div class="Article">
		...
		<c:forTokens items="${sbs }" delims="&" var="sb" varStatus="s">
			<tr bgcolor="white">
				<td>${NewList[s.index].id }</td>
				<td><a href="${pageContext.request.contextPath }/login/HandleArticleDetail?articleId=${NewList[s.index].id}">${NewList[s.index].title }</a></td>
				<td>${sb }</td>
				<td>${types[NewList[s.index].type] }</td>
				<td><b>${NewList[s.index].lastTime }</b></td>
				<td><a href="${pageContext.request.contextPath }/login/HandleArticleDelete?articleId=${NewList[s.index].id}">删除</a></td>
		</c:forTokens>
		</table>
	</div>
	<br >
